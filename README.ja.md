...
## 更新のポイント：GPT-6 Astra のベストプラクティスに対応

**GPT-6 Astra の能力を実際のプロジェクトでより十分に活かすために。** BuildOS は [OpenAI 公式のプロンプト指針](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-astra#prompting-best-practices)の 5 項目を、運用規範、Skills、`AGENTS.md` の連携方針に取り入れました。

- **主体的な実行と完遂**：既存の承認範囲で作業を進め、不要な再確認を減らします。
- **指示への追従**：ユーザー指示と Skill の指針の優先順位を明確にし、重複や矛盾を整理します。
- **表現と文体**：要点を先に伝え、文章のつながりを保ち、定型句を減らし、指定された言語と形式を尊重します。
- **サブエージェントへの委任**：ホストが許可し、時間短縮や品質向上が見込める場合に独立した作業を分担します。
- **テストと検証**：変更の影響とリスクに応じて確認範囲を選び、確実な成果につなげます。

これらの改善は、協働の摩擦と不要な文脈消費を減らし、問題の理解、実装、成果の提供により多くの注意を向けられるようにすることを目指しています。
