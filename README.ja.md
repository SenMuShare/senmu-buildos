...
Senmu BuildOS は、**AI coding agent** のためのオープンソース運用規範兼ソフトウェア工程コーチです。要件、インターフェース設計、技術設計、フレームワーク／コンポーネント選定、フロントエンド／バックエンド実装、テスト、Git、リリース、再利用可能な学習までを扱い、長くなる一方の Prompt に頼らず、実プロジェクトで継続的に成果を出せるようにします。
...
| 会話や Agent が変わるたびに説明し直し、長い規則を再読する | 判断、進捗、証拠をプロジェクト文書とタスクに残し、必要な規則だけを読み、有効な情報を再利用する |
...
- **インターフェースを実際のタスクに役立てる。** 視覚、操作、モーション、アクセシビリティを一体で設計し、スタイルの装飾でプロダクト事実を置き換えません。
...
Senmu BuildOS の現行正式リリースは `v2.1.0` です。Codex、Claude Code、豆包アダプターをサポートします。8 Skill は個別ではなく、1 つのプラグインとしてインストールします。
