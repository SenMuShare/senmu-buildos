...
Senmu BuildOS の現行正式リリースは `v1.14.0` です。**Codex と Claude Code** は同じ 7 つの Skill を共有し、それぞれ独立したプラグイン manifest、Marketplace、ライフサイクル Hook アダプターを持ちます。どちらのアダプターもユーザー全体の設定やプロジェクトファイルを書き換えず、ネットワークへ接続しません。フィードバック収集は高シグナル候補だけをローカルの `~/.senmu-buildos/feedback/` に保存し、通常の回答に内部マーカーや ID を表示しません。インストール、有効化、削除はユーザーが明示的に操作します。
