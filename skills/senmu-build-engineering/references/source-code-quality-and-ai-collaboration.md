...
8. A clear phase may use a local checkpoint commit while the Change Unit remains `in_progress`. For review, freeze a stable commit while the unit remains open. Seal only at authorized batch closeout after applicable checks and required review/repair under Delivery; review alone is not sealing. Do not fake closeout when scope is incomplete, checks fail, project rules prohibit it, or another contributor's changes would be mixed. Engineering may claim only `implemented`.
...
3. **Task Review:** begin with a lightweight Challenger Review that attempts to falsify direction, assumptions, omitted boundaries, and simpler alternatives. Trace shared upstream causes rather than fragmenting symptoms. Conclude on two independent axes: `Requirement/Spec` checks approved requirement, acceptance, scope, non-goals, and user-visible behavior; `Engineering/Standards` checks the real diff for correctness, safety, data, compatibility, full retirement, tests, and maintenance cost. Assess approved functional and nonfunctional requirements, architecture/cohesion/coupling/reuse, and applicable framework/design conventions before choosing additional tests. One passing axis cannot hide another failure. A Finding names axis, location, trigger, impact, severity, and recheck condition. Challenger is a method, not a job or mandatory extra agent; same-executor review remains self-review. Use Assurance rules for an independent conclusion.
4. **Scoped Re-review:** route repair by actual open/sealed/integrated state under Delivery’s [Review Repair Routing](../../senmu-build-delivery/references/multi-agent-change-units-and-version-line-closeout.md#7-review-repair-routing). Recheck the Finding, repair diff, and affected chain, not unrelated checklists. Register new problems separately; open blockers prevent approval.
5. **Final Review:** summarize both axes over complete `base..head`, closed Findings, matching tests, and blind spots. A new commit requires renewing the candidate conclusion over its delta and affected behavior; unaffected evidence remains reusable under the testing standard. Delivery may reuse the conclusion while frozen head remains unchanged.
...
Group ordinary Findings into a review round for the frozen head; report blockers early. A changed head requires a renewed overall conclusion, reusing unchanged evidence under the testing standard and checking the repair/affected chain. Close a small self-contained slice with early appropriate checks; neither run full gates on each unfinished edit nor postpone all checks until the entire version is built.
