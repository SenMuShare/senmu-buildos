...
Codex 项目默认是 `parallel-capable`：用户可以在当前修改结束前随时开启第二个会话，因此“眼下没有观察到其他写入者”不能证明单写安全。先判断是否存在覆盖整个修改窗口、由项目或 Harness 保证的独占写入，再判断改动大小：

| 现场 | 文案／小修 | 普通 Bug／功能 | 大型 POC／长期改造 |
| --- | --- | --- | --- |
| 默认 Codex 写入；未来会话数量未知 | 独立短分支 + worktree | 独立短分支 + worktree | 独立 POC／继任版本线 + worktree |
| 项目／Harness 保证整个修改窗口独占写入，且工作区干净 | 可在当前主线修改、匹配验证后立即 commit | 当前目录短分支 | 独立短分支或专项版本线 worktree |
...
直接主线只适用于 G1、本地可逆、基线干净、没有候选冻结、项目明确允许且独占写入保证覆盖整个修改窗口的改动；完成必须立即验证并 commit。默认 Codex 写入即使只是一个标点也使用独立短分支和 worktree；固定隔离成本换取后来新增会话时无需搬家、抢目录或猜归属。分支隔离提交历史，worktree 隔离物理写入；并行写入必须同时具备二者。
...
直接主线例外期间一旦出现第二个写入组，立即停止继续共享写入。当前改动若范围完整且归属明确，先验证并封口 commit；否则把可归属 patch 转移到独立 worktree。不得为搬家自动 stash、reset 或提交混合脏改动；无法拆分时标记 blocked 并做一次 owner 对账。

## 3. 无固定 Leader 的集成收口

收到“合并这批改动”或“发布最新版本”的当前 Agent 自动成为本次集成收口者：
