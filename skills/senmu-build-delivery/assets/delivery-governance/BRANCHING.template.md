...
- 共享工作区与脏工作树处理：`<待确认>`
- 协作执行面：默认一个权威事实源；只读并行不切分支，Codex 写入默认按未来可新增会话处理，每个任务自动使用独立短分支和 worktree。只有项目／Harness 保证整个修改窗口独占写入时才省略额外 worktree；项目禁止、复制成本显著或共享状态无法隔离时改为串行。
...
- 只读任务不得创建 worktree；写入任务默认创建短分支和 worktree，不等待用户预告并发数量。
