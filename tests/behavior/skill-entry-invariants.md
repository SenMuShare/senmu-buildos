...
| --- | --- | --- |
| S-01 | 面向用户的表达方式由 Host／Agent 的共同通信契约统一负责；七个专业入口不重复复制同一套写作风格正文，只保留会改变该专业判断或交付的特有规则 | Host／Agent instructions、七个 `skills/senmu-build-*/SKILL.md` |
...
| L-07 | Hook 只从用户本轮请求或批注捕获明确纠正／投递动作，不扫描引用会话、选中的 Assistant 文本、附件包装或宿主生成建议；仅询问反馈机制不入箱。命中后只落盘相关用户片段并返回空 Hook 输出，不向模型注入候选 ID、收集说明或处置指令。Agent 用本机 CLI 静默提交高信号候选。两条路径都去重脱敏，不自动触发 Learning 或修改项目 | `反馈候选与集中审议规范.md`、`docs/architecture/hook-lifecycle.md` |
