...
| --- | --- | --- |
| S-01 | Agent 与用户对话的表达方式由 Host／Agent 共同通信契约负责；产品界面的按钮、状态、错误和提示由 Product 内容规范负责。七个专业入口不复制通用对话风格或界面文案正文，只保留会改变本专业判断的边界和路由 | Host／Agent instructions、Product 内容规范、七个 `skills/senmu-build-*/SKILL.md` |
...
| E-30 | Engineering 按项目产品内容标准实现界面；Ant Design 或其他组件库只拥有组件与交互实现，默认“确定／取消”和错误文本不是跨页面文案 owner | `frontend-ant-design-practice.md`、`docs/architecture/skill-boundaries.md` |
