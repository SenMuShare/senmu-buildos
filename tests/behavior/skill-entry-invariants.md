...
| L-07 | Hook 只捕获明确纠正或投递动作，仅询问反馈机制不入箱；命中后只落盘并返回空 Hook 输出，不向模型注入候选 ID、收集说明或处置指令。Agent 用本机 CLI 静默提交高信号候选。两条路径都去重脱敏，不自动触发 Learning 或修改项目 | `反馈候选与集中审议规范.md`、`docs/architecture/hook-lifecycle.md` |
